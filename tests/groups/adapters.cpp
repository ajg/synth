...
unit_test(assignments) {
    ...
    context[text::literal("this_value")] = 175;
    context[text::literal("max_value")] = 200;

    context[text::literal("xml_var")] = text::literal("<foo><bar qux='ha'>&</bar></foo>");
    context[text::literal("a_few_lines")] = text::literal("this\nhas\nlines");
    context[text::literal("more_lines")] = text::literal("this\nhas\nlines\n\r\nand\nsome\nmore");
    context[text::literal("a_bin_string")] = text::literal("this \f string \t contains \b binary \0 indeed");
    ...
    /// context[text::literal("a_float_pointer")] = value(float_pointer, 6);

    scoped_ptr<vector<int> > scoped_(new vector<int>(3, 3));
    auto_ptr<vector<int> > auto_ptr_(new vector<int>(4, 4));

    context[text::literal("X")] = text::literal("this"); // X;
    context[text::literal("Y")] = boost::ref(Y);
    context[text::literal("Z")] = Two;
    context[text::literal("some_strings")] = some_strings;
 // TODO: context[text::literal("a_set"] = set<string_type, string_type>();
    context[text::literal("an_int_vector")] = doodoo;
    context[text::literal("a_shared_ptr")] = boost::shared_ptr<vector<int> >(new vector<int>(6, 6));
    context[text::literal("a_scoped_ptr")] = scoped_;
    context[text::literal("a_string_array")] = string_array;
    context[text::literal("tag_list")] = tag_list;
    context[text::literal("a_float_array")] = float_array;
    context[text::literal("an_auto_ptr")] = auto_ptr_;
    context[text::literal("a_char")] = char_type('z');
    context[text::literal("an_int")] = 12;
    ...
    context[text::literal("a_scoped_array")] = value_type(a_scoped_array, 5);

    map<int, int> a_map = assign::map_list_of(1, 2)(3, 4);
    context[text::literal("a_deque")] = deque<char_type>();
    context[text::literal("a_map")] = a_map;

    vector<vector<vector<int> > > _vectors;
    context[text::literal("some_vectors")] = _vectors;
    context[text::literal("shared_array")] = value_type(shared_array<string_type>(), 0);

    context[text::literal("a_complex_float")] = complex<float>();
    context[text::literal("a_pair")] = make_pair(1, 2.5);
    context[text::literal("a_boost_array")] = boost::array<double, 2>();
    context[text::literal("an_optional_float")] = optional<double>(4.5);

    // FIXME:
    // auto_ptr<vector<bool> > bools_(new vector<bool>(2, false));
    // vector<bool> *const bools = bools_.get();
    // context[text::literal("a_pointer")] = bools;

    context[text::literal("a_true")] = true;
    context[text::literal("a_false")] = false;

    string_type sss = text::literal("This should be a string.");
    char_type const *const ccc = sss.c_str();
    context[text::literal("char_pointer")] = ccc;
}}}
